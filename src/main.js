...
if (nullcheck === undefined || nullcheck === null) { // checks if TorexV3 is already open
...
if (scriptLocationOnLoad == originalScript) { // checks if the script has already been loaded, if it has page gets reloaded
    try { radiantModal("Error!", "Please run TorexV3 before opening lessons.", goToDashboard) } // unused as of right now
    catch (error) { alert('Please run TorexV3 before opening lessons. Reloading...'), location.reload() }
} else {
    // override the script
    fetch(modifedScript).then(r => r.text()).then(r => eval(r))
}
} else { // reloads i-ready if a lesson is detected
  alert('Please run TorexV3 before opening lessons. Reloading...'), location.reload()
}
} else { // prevents TorexV3 from opening if it's already open
  console.log(TorexV3 is already open.')
}
